...
### deaf HS attainment/age trend?
hsByAge <- dat%>%filter(deaf=='deaf')%>%mutate(hs=attainCum>='HS Diploma')%>%group_by(agep)%>%
    summarize(hs=svmean(hs,pwgtp))
trendTest <- with(hsByAge,cor.test(agep,hs,method='spearman'))
#################################################################################################
#### Employment, LFP
#################################################################################################
...
### can you tell me what the 2017 employment rates are for deaf and hearing when we combine MA and Phd into one category (i.e., ­> MA)
postBAemp <- dat%>%filter(attainCum>'Bachelors')%>%group_by(deaf)%>%do(x=empFun(1,.))
ttt <- FIXsig(postBAemp)
knitr::kable(ttt[,grep('%',colnames(ttt))])
